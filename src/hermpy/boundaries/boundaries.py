...
def load_crossings(
    ...
    if backend.lower() == "philpott":
        crossings = _reformat_philpott(path, include_data_gaps=include_data_gaps)

    elif backend.lower() == "sun":
        crossings = _reformat_sun(path)
...
def _reformat_sun(input_directory: str) -> pd.DataFrame:
...
def _reformat_philpott(input_path: str, include_data_gaps=True) -> pd.DataFrame:
